...
%Personnages susceptibles de tuer P : P1 peut tuer P2 ? ou P1 peut tuer ...
    ...
    %tuer par sniper
    peutTuer(P1,P2):-personnage(P1,X,Y,_,_),caseSniper(X,Y), personnage(P2,X,_,_,_). %rajouter le fait que P1 doit être seul sur sa case
    peutTuer(P1,P2):-personnage(P1,X,Y,_,_),caseSniper(X,Y), personnage(P2,_,Y,_,_). %rajouter le fait que P1 doit être seul sur sa case
    
    %tuer par pistolet
    peutTuer(P1,P2):-voisinGauche(P1,LP),member(personnage(P2,_,_,_,_),LP). %rajouter le fait que P1 doit être seul sur sa case
    peutTuer(P1,P2):-voisinDroit(P1,LP),member(personnage(P2,_,_,_,_),LP). %rajouter le fait que P1 doit être seul sur sa case
    peutTuer(P1,P2):-voisinHaut(P1,LP),member(personnage(P2,_,_,_,_),LP). %rajouter le fait que P1 doit être seul sur sa case
    peutTuer(P1,P2):-voisinBas(P1,LP),member(personnage(P2,_,_,_,_),LP). %rajouter le fait que P1 doit être seul sur sa case